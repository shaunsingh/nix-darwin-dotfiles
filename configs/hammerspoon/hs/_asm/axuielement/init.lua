	...
	if type(key) == "string" then
			...
			for _, v in ipairs(objectMT.actionNames(self) or {}) do
				if v == AXName then
					return function(self2, ...)
						return objectMT.performAction(self2, v, ...)
					end
				end
			end
		elseif parameterized then
			for _, v in ipairs(objectMT.parameterizedAttributeNames(self) or {}) do
				if v == AXName then
					return function(self2, ...)
						return objectMT.parameterizedAttributeValue(self2, v, ...)
					end
				end
			end
		else
...
objectMT.__pairs = function(self)
	...
	-- iteration
	for _, v in ipairs(objectMT.attributeNames(self)) do
		keys[v] = true
	end
...
objectMT.matchesCriteria = function(self, criteria)
	...
	for idx, thisCriteria in ipairs(criteria) do
		...
		if thisCriteria.attribute then
			if type(thisCriteria.attribute) ~= "table" then
				thisCriteria.attribute = { thisCriteria.attribute }
			end
		end
		if thisCriteria.action then
			if type(thisCriteria.action) ~= "table" then
				thisCriteria.action = { thisCriteria.action }
			end
		end
		if thisCriteria.parameterizedAttribute then
			if type(thisCriteria.parameterizedAttribute) ~= "table" then
				thisCriteria.parameterizedAttribute = { thisCriteria.parameterizedAttribute }
			end
		end
		if thisCriteria.value then
			if type(thisCriteria.value) ~= "table" then
				thisCriteria.value = { thisCriteria.value }
			end
		end
	end
...
module.searchCriteriaFunction = function(...)
	...
	return function(e)
		return e:matchesCriteria(table.unpack(args))
	end
end
...
local elementFilterHamster = function(self, elementFilterObject)
	...
	for _, v in ipairs(self) do
		...
		if state.callback and coroutine.isyieldable() then
...
elementSearchResultsFilter = function(self, criteria, callback)
	assert(type(criteria) == "function" or (getmetatable(criteria) or {}).__call, "expected function for criteria")
...
local elementSearchHamsterBF = function(elementSearchObject)
	...
	local seen = esoMT._seen
		or setmetatable({ [self] = {} }, { -- capture initial self
	...
	while #queue > 0 do
		if state.cancel or maxDepth < depth or count == maxCount then
			break
		end
		...
		if getmetatable(element) == objectMT then
			...
			if type(queue[#queue]) ~= "table" then
				table.insert(queue, {})
			end
	...
	if asTree and criteriaEmpty and not objectOnly then
		results = results[1]
	end
