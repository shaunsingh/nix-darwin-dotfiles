...
* Intro
Note: half of this config is stolen from tecosaur, go look at his instead. This is trash
...
1. I use neovim, not vim.
2. Even if I found a neovim alternative, you can't do everything.
...
** Why Emacs?
(right now stolen from tecosaurs config, I'll fix it later)
...
** Themes
I. Love. Themes. Which is why I have so many. I switch between themes pretty often, so lets just leave them all here for now and uncomment the ones I need
#+begin_src elisp
;;doom themes
...
I think the defualt thicc toolbar looks quite nice on macOS. Lets keep that enabled against dooms will
#+begin_src elisp
;;enable toolbar
(tool-bar-mode 1)
#+end_src

I like transparency, but it doesn't look too good. for now lets leave it commented out
#+begin_src elisp
;;transparency
...
While we're at it, lets make emacs scroll by visual lines instead of actual ones, so its much easier to work with wrapsjkA
#+begin_src elisp
...
I don't like seeing the cursorline, especialy while writing. Lets disable that
#+begin_src elisp
;;disable cursorline
...
I like using the minimap, even if its slow. Looks cool in my opinion. Anyways, lets make it a little cooler by removing the scroll highlighting
#+begin_src elisp
...
Centaur tabs look pretty good out of the box. I like to keep inactive panes grey, so lets make inactive tabs grey as well
#+begin_src
;;tabs
...
I want to have ivy ask me what file I want to see after creating a split. Lets adjust the ivy search a bit, and put it at the top
#+begin_src elisp
...
;;yasnippet
(setq yas-triggers-in-field t) ;;nested snippets
...
I like a little padding on my org blocks, just a millimeter or two on the top and bototm should do 
#+begin_src elisp
;;add padding in org
