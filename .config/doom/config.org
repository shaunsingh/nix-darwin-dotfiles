...
* Modules
Doom emacs has several modules, which I would like to enable/disable depending
on what I'm working on. Below is my current config
...
On the other hand, don't byte compile the =packages.el= file
#+begin_src elisp
;; -*- no-byte-compile: t; -*-
#+end_src
...
** Themes
Always grab the latest doom themes
#+begin_src emacs-lisp :tangle packages.el
(unpin! doom-themes)
#+end_src

I dislike solaire mode, so lets disable that
#+begin_src emacs-lisp :tangle packages.el
...
** LSP
Always grab the latest lsp stuff
#+begin_src emacs-lisp :tangle packages.el
(unpin! lsp-ui)
(unpin! lsp-mode)
#+end_src
...
I like a bit of padding on the left hand side, and lets make the line spacing
comfier
...
** Modeline
Always grab the latest modeline package
#+begin_src elisp :tangle packages.el
(unpin! doom-modeline)
#+end_src
...
Treemacs also has a habit of loading the git stuff too soon, lets fix that
...
Of course we don't need that to show all the time, so lets hide and show it when
needed
#+begin_src emacs-lisp :tangle packages.el
(package! org-appear)
#+end_src
...
** Org-Roam
Install org-roam-ui and its dependencies
#+begin_src elisp :tangle packages.el
...
Org-super-agenda!
#+begin_src elisp :tangle packages.#+begin_src emacs-lisp :tangle yes
...
Obviously we can't edit a png though. Let use org-fragtog to toggle between
previews and text mode
#+begin_src elisp :tangle packages.el
(package! org-fragtog)
#+end_src
