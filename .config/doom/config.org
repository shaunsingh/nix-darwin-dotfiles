...
For mixed pitch, I would go with something comfier. I like IBM Plex Sans, so lets go with that
#+begin_src elisp
;;mixed pitch modes
...
I'm trying out some ui stuff too
...
** Monkeytype
Just a few tweaks, making it insert my default, etc
#+begin_src elisp
...
requires a custom (non-cached) build of emacs, I prefer to use webkit which is a
tad bit outdated but works
#+begin_src emacs-lisp
;; (use-package org
;;   :ensure t)
